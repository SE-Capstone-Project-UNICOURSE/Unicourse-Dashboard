...
const useCourseMentorEditFormFields = (): FormFieldConfig<courseMentorEditFormValuesTypes>[] => {
  const { data } = useAppSelector((state) => state.listCourseOfflineLecture.centers);
  const dispatch = useAppDispatch();

  const {
    formState: { errors },
  } = useFormContext<courseMentorEditFormValuesTypes>();

  const handleUpdatePreviewImage = (imageUrl: string) => {
    dispatch(setPreviewImage(imageUrl));
  };

  const config: FormFieldConfig<courseMentorEditFormValuesTypes>[] = [
    {
      name: 'title',
      label: 'Tiêu đề',
      inputType: 'input',
...
export default useCourseMentorEditFormFields;
